...
public partial class SlimeAI : EnemyAI
{
	...
	// FIX: Added 'f' to float literals
	[Export] public float IdleJumpInterval { get; set; } = 3.0f; // Jump every 3 seconds when idle
	[Export] public float AggroJumpInterval { get; set; } = 1.0f; // Jump every 1 second when chasing
	[Export] public float JumpTowardPlayerChance { get; set; } = 0.7f; // 70% chance to jump toward player
	...
	public override void Initialize(Enemy enemy)
	{
		...
		// It's good practice to randomize the seed once, although Godot 4+ handles this automatically.
		_random.Randomize();
	}

	public override void ProcessAI(double delta)
	{
		base.ProcessAI(delta); // Run normal AI state machine
	...
	private void PerformJump(bool hasTarget)
	{
		...
		{
			// Jump toward player
			float directionToPlayer = Mathf.Sign(_enemy.CurrentTarget.Position.X - _enemy.Position.X);
